...
## ERR-015: rsync timeout(exit 20) 반복 및 SMB 한글 파일명 전송 실패

- **발생일**: 2026-07-08
- **증상**:
  - 09시부터 매시 정각 동기화에서 `rsync(XXXXX): error: poll: timeout` (exit code 20)이 반복됨
  - NAS 마운트는 정상이고 dry-run도 성공하지만, 실제 전송 시 timeout 발생
  - 410개 파일(185MB)이 밀려 있었으며, SMB 위에서 `--timeout=60`으로는 대량 전송을 완료할 수 없었음
  - timeout 해결 후 exit code 23 발생 — `★` 등 특수 유니코드 문자가 포함된 한글 파일명에서 `mkstempat: Invalid argument` 오류
- **원인**:
  - `--timeout=60`은 I/O 무응답 60초 시 rsync를 종료시킴. 대량 파일 전송 중 SMB 응답 지연이 이 임계값을 초과
  - NAS SMB 파일시스템이 특수 유니코드 문자(★)나 긴 한글 경로의 임시 파일 생성을 지원하지 않음
- **해결**:
  - `--timeout=60` → `--timeout=300`으로 확대
...
- **영향 파일**:
  ...
  - `docs/history.md`
- **검증**:
  - timeout 300초로 변경 후 수동 실행: 밀린 파일 전송 완료
  - exit 23 허용 후 수동 실행: `전송: 1개, 실패: 1개 - 파일명 호환 문제` 정상 처리
  - 14일 경과 파일 2,368개 정리 완료
- **교훈**: SMB 위 rsync timeout은 예상 전송량에 비례하여 넉넉하게 설정해야 한다. 특수 유니코드 파일명은 NAS SMB에서 처리 불가할 수 있으므로 exit 23을 허용하고 실패 파일만 로깅하는 것이 실용적이다.

---

## ERR-016: Slack 웹훅 알림 미발송 (코드 반영 시점 불일치)

- **발생일**: 2026-07-08
- **증상**: Slack 알림 코드를 추가했지만, 이전 시간대 실행은 수정 전 스크립트로 실행되어 알림이 가지 않음
- **원인**: launchd가 이미 로드한 스크립트는 파일 수정 즉시 반영되지만, 수정 전에 실행된 프로세스에는 적용되지 않음
- **해결**: 수동 실행으로 Slack 알림 정상 발송 확인
- **영향 파일**: `~/sync_to_nas.sh`
- **교훈**: 운영 스크립트 수정 후에는 수동 실행으로 반영 여부를 즉시 확인하는 것이 좋다.
